...
try:
    ...
    logger.error("❌ Playwright 未安装，请先安装：")
...
async def douyin_login(cookies_path: str = None, persist: bool = False):
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            for i in range(60):
                ...
                if has_strong_login:
                    ...
                    if (i + 1) % 5 == 0:
                        logger.info(f"   等待登录中... ({i+1}/60)")

            # 提取抖音相关的 cookies
            dy_cookies = {}
            if cookies:
                for cookie in cookies:
                    if "douyin.com" in cookie.get(
                        "domain", ""
                    ) or ".douyin.com" in cookie.get("domain", ""):
            ...
            if not persist:
                ...
                logger.info("   💾 持久化模式：浏览器状态已保存，下次无需扫码")
                logger.info("   提示：首次使用需要扫码，后续将自动使用已保存的登录状态")
        ...
        if cookie_str:
            ...
            if not success:
                logger.warning(f"⚠️ 提取到的 Cookie 验证未通过: {msg}")
                logger.info("这可能会影响后续的数据抓取，请考虑重新扫码。")
            else:
                logger.info("✅ Cookie 格式及有效性验证通过")
        ...
        if cookies_path:
            ...
            logger.info(f"   💾 已保存到：{cookies_path}")
...
async def main():
    ...
    parser.add_argument(
        "--output", "-o", help="输出配置文件路径（保存到 config.cookie 字段）"
    )
