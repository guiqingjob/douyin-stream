...
| 任务 | 预计时间 | 负责人 |
|------|----------|--------|
| P0-1: 消除代码重复 | 30 分钟 | Agent |
| P0-2: 修复 orchestrator_v2.py Bug | 10 分钟 | Agent |
| P0-3: 修复 Web 页面旧路径引用 | 15 分钟 | Agent |
| P0-4: 调用任务进度组件 | 10 分钟 | Agent |

**验收标准**:
- [ ] `scripts/core/` 目录已删除或创建符号链接
- [ ] `orchestrator_v2.py` 批量转写不再崩溃
- [ ] 所有 Web 页面使用统一的 `media_tools.douyin.core.*` 路径
- [ ] 下载/转写页面显示进度条
...
| 任务 | 预计时间 | 负责人 |
|------|----------|--------|
| P1-1: task_queue.py 绝对路径 | 5 分钟 | Agent |
| P1-2: 创建 accounts.json | 2 分钟 | Agent |
| P1-3: 改进删除用户交互 | 15 分钟 | Agent |
| P1-4: 批量操作实时进度 | 1 小时 | Agent |
| P1-5: 错误历史查看 | 1 小时 | Agent |
...
| 任务 | 预计时间 | 负责人 |
|------|----------|--------|
| P2-1: 重构 stats_panel 缓存 | 15 分钟 | Agent |
| P2-2: 统一 Cookie/认证管理 | 1 小时 | Agent |
| P2-3: 添加新手引导 | 2 小时 | Agent |
| P2-4: 使用 st.navigation | 30 分钟 | Agent |
| P2-5: 数据导入/导出/备份 | 1 小时 | Agent |

**验收标准**:
- [ ] stats_panel.py 缓存逻辑简化
- [ ] 认证体系统一
- [ ] 首次访问显示引导
- [ ] 使用 Streamlit 原生导航
- [ ] 支持数据备份/恢复
