...
### 重构脉络
[docs/pipeline_reliability_refactor.md](docs/pipeline_reliability_refactor.md) 是 pipeline 改造的完整设计文档，**第一到第四阶段全部落地（2026-05-05）**：视频级状态治理 → 可恢复转写流水线 → 可观测性（失败聚合 / 健康检查 / PARTIAL_FAILED / 日志归档）。改 pipeline / transcribe 相关代码前必读，尤其是第 156-189 行的"Phase 3 已完成机制"——续传不变量在那里。
