...
- ✅ 清除历史后不再恢复（前端 historyCleared 标记）
- ✅ WebSocket 断连提示（红色提示条 + 红点）
- ✅ 简化重试按钮（只保留一个"重试"按钮）
- ✅ 展开详情面板显示子任务状态
- ✅ 状态标签友好化（"可能中断"替代"已过期"）
- ✅ 后端 payload 支持结构化进度信息
- ✅ 子任务列表展示成功/失败/进行中
- ✅ 清理宽泛异常捕获（从 56 处减少到 38 处）
...
| `/inbox` | `Inbox.tsx` | 收件箱：三栏布局（创作者列表+素材列表+预览面板），支持搜索/导出/标记，自动同步文件系统 |
...
### 7.4 已完成 — 2026-04-15 ~ 2026-04-20

- [x] **收件箱三栏布局重构**：Apple Mail Pro 风格，创作者列表+素材列表+即时预览
- [x] **本地文件夹分组**：本地上传素材按文件夹分组显示，支持展开/折叠
- [x] **数据库自动同步**：进入 Inbox 页面自动触发 `reconcile_transcripts` 同步
- [x] **Apple 设计语言**：毛玻璃效果、Spring 动画、语义化配色
- [x] **双向同步完善**：删除不存在的本地创作者、迁移孤儿素材、清理空创作者
- [x] **批量操作优化**：勾选多条素材后支持批量删除、批量已读、导出
