...
### Q3: 运行 `login.py` 时报错 `Playwright 未安装` 或 `Executable doesn't exist`
- **原因**：缺少 Playwright 的核心包或 Chromium 浏览器内核。
- **解决方案**：
  依次执行以下命令：
  ```bash
...
### Q4: 运行 `compress.py` 报错 `未找到 ffmpeg`
...
### Q6: 为什么打开 Web 界面 (`index.html`) 数据没有更新？
