...
| 序号 | 任务 | 文件 | 预计改动 |
|------|------|------|----------|
| 1.1 | 实现数据库清理逻辑 | `web/pages/cleanup.py` | +30 行 |
| 1.2 | 添加任务队列锁 | `web/components/task_queue.py` | +10 行 |
| 1.3 | 临时文件自动清理 | `web/pages/transcribe.py` | +15 行 |

**验收标准**:
- [ ] 数据库清理功能真正执行了清理操作，并显示统计
- [ ] 并发更新任务状态时无数据丢失
- [ ] 转写完成后临时文件被删除
...
| 序号 | 任务 | 文件 | 预计改动 |
|------|------|------|----------|
| 2.1 | 批量下载错误统计 | `web/pages/download.py` | +20 行 |
| 2.2 | 批量转写错误统计 | `web/pages/transcribe.py` | +20 行 |
| 2.3 | 创建 constants.py | `web/constants.py` (新文件) | +15 行 |
| 2.4 | 替换所有硬编码路径 | 4 个页面文件 | 修改 10+ 处 |
| 2.5 | 任务取消机制 | `web/components/task_queue.py` | +30 行 |
| 2.6 | 安全 JSON 序列化 | `web/components/progress_display.py` | +10 行 |

**验收标准**:
- [ ] 批量操作显示成功/失败统计报告
- [ ] 用户可以取消正在运行的任务
- [ ] 所有路径从 constants.py 统一管理
- [ ] 复杂对象显示不会导致页面崩溃
...
| 序号 | 任务 | 文件 | 预计改动 |
|------|------|------|----------|
| 3.1 | 删除操作确认 | `web/pages/following.py` | +5 行 |
| 3.2 | 异常日志记录 | 多处 | 修改 5+ 处 |
| 3.3 | 统计面板缓存 | `web/components/stats_panel.py` | +5 行 |
| 3.4 | 提取文件大小格式化 | `web/utils.py` (新文件) | +10 行 |
| 3.5 | 统一认证检查调用 | 3 个页面文件 | 删除重复代码 |
...
| 序号 | 任务 | 文件 | 预计改动 |
|------|------|------|----------|
| 4.1 | 预设模板外部化 | `config.yaml` + `settings.py` | +20 行 |
| 4.2 | 代码审查和优化 | 全部 | 重构 |
...
### 阶段 1 检查
- [ ] P0-1: `web/pages/cleanup.py` 数据库清理是否已实现？
- [ ] P0-2: `web/components/task_queue.py` 是否添加了 `_state_lock`？
- [ ] P0-3: `web/pages/transcribe.py` 临时文件是否已清理？

### 阶段 2 检查
- [ ] P1-1: `web/pages/download.py` 批量下载是否有错误统计？
- [ ] P1-1: `web/pages/transcribe.py` 批量转写是否有错误统计？
- [ ] P1-2: 是否创建了 `web/constants.py`？
- [ ] P1-2: 所有硬编码路径是否已替换为常量？
- [ ] P1-4: `web/components/task_queue.py` 是否支持取消？
- [ ] P1-5: `st.json()` 调用是否有安全包装？

### 阶段 3 检查
- [ ] P2-1: `web/pages/following.py` 删除操作是否有确认？
- [ ] P2-2: 所有 `except Exception: pass` 是否添加了日志？
- [ ] P2-3: `web/components/stats_panel.py` 是否有缓存？
- [ ] P1-2: 重复代码（认证检查、环境检测）是否已统一？
- [ ] P1-2: 文件大小格式化是否提取为公共函数？
